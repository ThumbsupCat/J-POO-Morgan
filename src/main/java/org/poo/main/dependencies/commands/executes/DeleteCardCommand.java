            ...
            for (Account account : user.getAccounts()) {
                for (Card card : account.getCards()) {
                    if (card.getCardNumber().contentEquals(input.getCardNumber())) {
                        /* Found the card, we add the transaction
                        && taking the object for safe deletion */
                        user.getTransactions().add(
                                new CardTransaction(account.getIBAN(), input.getCardNumber(),
                ...
                if (targetedCard != null) {
                    /* Removing the card from the account's card list */
